...
/**
 * Tenant-scoping Prisma Client Extension for the `invoice` model.
 *
 ...
 * clause still cannot leak another tenant's rows, because the
 * extension injects the filter regardless of what the caller passed.
 *
 ...
 * instead, which this extension does scope.
 */
    ...
    query: {
      invoice: {
        async $allOperations({ operation, args, query }) {
          ...
          if (!store) {
            // No authenticated request in scope (e.g. seed script, tests
            // that opt out deliberately) — fall through unscoped.
            return query(args);
          }
          ...
          if (
            READ_OR_WRITE_ONE_WHERE_OPS.has(operation) ||
            MANY_WHERE_OPS.has(operation)
          ) {
